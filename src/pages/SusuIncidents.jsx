...
const ALERT_META = {
  ADMIN_DEFAULT: { label: 'Admin Default', color: 'bg-[var(--f-bad-bg)] text-[var(--f-bad)]', icon: ShieldAlert },
  MASS_DEFAULT_THRESHOLD: { label: 'Mass Default', color: 'bg-[var(--f-bad-bg)] text-[var(--f-bad)]', icon: Siren },
  ESCROW_DIVERSION: { label: 'Escrow Diversion', color: 'bg-[var(--f-warn-bg)] text-[var(--f-warn)]', icon: ArrowDownToLine },
...
function AlertCard({ alert, onResolve }) {
  ...
  return (
    <div className={`bg-[var(--f-surface-raised)] border rounded-xl p-4 ${resolved ? 'border-line' : 'border-[var(--f-bad)]'}`}>
        ...
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {resolved
                ? <Tag className="bg-[var(--f-ok-bg)] text-[var(--f-ok)] border-0 text-xs">{alert.resolution}</Tag>
                : alert.acknowledgedAt
                  ? <Tag className="bg-[var(--f-warn-bg)] text-[var(--f-warn)] border-0 text-xs">ACKNOWLEDGED</Tag>
                  : <Tag className="bg-[var(--f-bad-bg)] text-[var(--f-bad)] border-0 text-xs">OPEN</Tag>}
            </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-[var(--f-bad-bg)] rounded-lg flex items-center justify-center">
            <Siren className="w-4 h-4 text-[var(--f-bad)]" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-[var(--f-text)]">War Room — Susu Incidents</h1>
            <p className="text-sm text-ink-2">
      ...
      <div className="flex gap-1.5">
        {FILTERS.map((f) => (
          <button key={f.key} onClick={() => setFilter(f)}
            className={`text-xs px-3 py-1.5 rounded-lg border transition-colors ${filter.key === f.key ? 'border-emerald-500 bg-[var(--f-ok-bg)] text-[var(--f-ok)]' : 'border-line text-ink-2 hover:border-line-bright'}`}>
            {f.label}
          </button>
        ))}
      </div>
