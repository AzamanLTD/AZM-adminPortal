...
const SCOPE_COLORS = { ALL: 'bg-[var(--az-blue-soft)] text-[var(--f-info)]', HOLIDAY: 'bg-[var(--az-amber-soft)] text-[var(--f-warn)]', CUSTOM: 'bg-[var(--az-violet-soft)] text-[var(--az-violet)]' };
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={openNew} className="bg-emerald-600 hover:bg-[var(--f-ok)] text-[var(--az-text-primary)]">
          <Plus className="w-4 h-4 mr-2" /> New Profile
        </Button>
      </div>

      {/* Priority chain explanation */}
      <div className="bg-[var(--az-surface-2)] border border-line rounded-xl p-4 flex items-center gap-3 text-xs text-ink-2">
        <Zap className="w-4 h-4 text-[var(--f-ok)] flex-shrink-0" />
        <span>Resolution order: <strong className="text-[var(--az-violet)]">CUSTOM (user-specific)</strong> → <strong className="text-[var(--f-warn)]">HOLIDAY (time-windowed)</strong> → <strong className="text-[var(--f-info)]">ALL (global override)</strong> → <strong className="text-ink-2">Hardcoded fallback</strong>. Highest priority number wins within same scope.</span>
      </div>
      ...
      <div className="space-y-3">
        ...
        {profiles.map((p) => (
          <div key={p.id} className="bg-[var(--az-surface-2)] border border-line rounded-xl p-4">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-xs bg-[var(--az-surface-3)] text-ink-2 px-2 py-0.5 rounded-full">Priority {p.priority}</span>
                  {!p.isActive && <span className="text-xs bg-[var(--az-red-soft)] text-[var(--f-bad)] px-2 py-0.5 rounded-full">Inactive</span>}
                </div>
              ...
              <div className="flex gap-2 flex-shrink-0">
                <Button variant="ghost" size="sm" onClick={() => openEdit(p)} className="text-ink-2 hover:text-[var(--az-text-primary)] hover:bg-[var(--az-surface-3)] h-8 w-8 p-0">
                  <Edit2 className="w-3.5 h-3.5" />
                </Button>
                <Button variant="ghost" size="sm" onClick={() => deleteMutation.mutate(p.id)} className="text-[var(--f-bad)] hover:text-red-300 hover:bg-[var(--az-red-soft)] h-8 w-8 p-0">
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-[var(--az-surface-2)] border border-line rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={submit} className="space-y-4">
              <div>
                ...
                <Input value={form.name} onChange={(e) => setF('name', e.target.value)} required className="bg-[var(--az-surface-3)] border-line text-[var(--az-text-primary)]" placeholder="e.g. Holiday Discount" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-ink-2 block mb-1">Scope</label>
                  <select value={form.targetScope} onChange={(e) => setF('targetScope', e.target.value)} className="w-full bg-[var(--az-surface-3)] border border-line rounded-lg px-3 py-2 text-sm text-[var(--az-text-primary)]">
                    {SCOPES.map((s) => <option key={s}>{s}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs text-ink-2 block mb-1">Priority</label>
                  <Input type="number" value={form.priority} onChange={(e) => setF('priority', e.target.value)} className="bg-[var(--az-surface-3)] border-line text-[var(--az-text-primary)]" />
                </div>
              </div>
              {form.targetScope === 'CUSTOM' && (
                <div>
                  <label className="text-xs text-ink-2 block mb-1">Target User IDs (comma-separated)</label>
                  <Input value={form.targetValue} onChange={(e) => setF('targetValue', e.target.value)} className="bg-[var(--az-surface-3)] border-line text-[var(--az-text-primary)]" placeholder="42,67,104" />
                </div>
              )}
              {form.targetScope === 'HOLIDAY' && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input type="date" value={form.validFrom} onChange={(e) => setF('validFrom', e.target.value)} className="bg-[var(--az-surface-3)] border-line text-[var(--az-text-primary)]" />
                  </div>
                  <div>
                    <label className="text-xs text-ink-2 block mb-1">Valid Until</label>
                    <Input type="date" value={form.validUntil} onChange={(e) => setF('validUntil', e.target.value)} className="bg-[var(--az-surface-3)] border-line text-[var(--az-text-primary)]" />
                  </div>
                </div>
              )}
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input type="number" step="0.01" value={form.platformFeePct} onChange={(e) => setF('platformFeePct', e.target.value)} className="bg-[var(--az-surface-3)] border-line text-[var(--az-text-primary)]" />
                </div>
                <div>
                  <label className="text-xs text-ink-2 block mb-1">Exit Fee %</label>
                  <Input type="number" step="0.01" value={form.exitFeePct} onChange={(e) => setF('exitFeePct', e.target.value)} className="bg-[var(--az-surface-3)] border-line text-[var(--az-text-primary)]" />
                </div>
                <div>
                  <label className="text-xs text-ink-2 block mb-1">Admin Split %</label>
                  <Input type="number" step="0.01" value={form.adminSplitPct} onChange={(e) => { setF('adminSplitPct', e.target.value); setF('vendorSplitPct', (100 - parseFloat(e.target.value || 0)).toFixed(2)); }} className="bg-[var(--az-surface-3)] border-line text-[var(--az-text-primary)]" />
                </div>
                <div>
                  <label className="text-xs text-ink-2 block mb-1">Vendor Split % (auto)</label>
                  <Input type="number" step="0.01" value={form.vendorSplitPct} onChange={(e) => { setF('vendorSplitPct', e.target.value); setF('adminSplitPct', (100 - parseFloat(e.target.value || 0)).toFixed(2)); }} className="bg-[var(--az-surface-3)] border-line text-[var(--az-text-primary)]" />
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <Button type="submit" disabled={!splitValid} className="flex-1 bg-emerald-600 hover:bg-[var(--f-ok)] text-[var(--az-text-primary)]">
