...
export function Field({ label, hint, error, required = false, children, className = '' }) {
  ...
  return (
    <div className={cn('f-field', className)}>
      {label && <label htmlFor={id}>{label}{required && <span className="text-bad ml-0.5" aria-hidden>*</span>}</label>}
      {child}
      {error ? <span className="f-field__err" role="alert">{error}</span> : hint && <span className="f-field__hint">{hint}</span>}
    </div>
  );
}
