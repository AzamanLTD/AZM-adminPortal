...
export default function NotificationCenter({ open, onOpenChange, notifications }) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="bg-bg border-line text-ink-primary w-full sm:max-w-md p-0 flex flex-col"
      >
        <SheetHeader className="p-4 border-b border-line">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs text-ink-2 hover:text-[var(--f-ok)] disabled:opacity-40 disabled:hover:text-ink-2 inline-flex items-center gap-1.5"
            >
        ...
        <ScrollArea className="flex-1">
          <div className="p-3 space-y-2">
            {isLoading && list.length === 0 && (
              <p className="text-ink-3 text-sm text-center py-8">Loading…</p>
            )}
            {list.map((n) => (
