export default function PoolBar({ label, balance, currency = 'USDC', max = 100000, status }) {
  ...
  const barSoft  = status === 'WARNING' ? 'var(--f-warn-bg)' : status === 'CRITICAL' ? 'var(--f-bad-bg)' : 'var(--f-ok-bg)';
    ...
    <div className="az-card rounded-lg p-3">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm font-bold f-mono" style={{ color: 'var(--f-text)' }}>
      ...
      <div className="w-full h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--f-surface-sunken)' }}>
        <div
